...
async function runTest () {
  ...
  const fileRelativePath ='../eth-stuffs'

  for (const file of await fs.readdir(path.join(__dirname, fileRelativePath))) {
    const ethBlock = await fs.readFile(path.join(__dirname, fileRelativePath, file))
